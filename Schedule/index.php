...
if(isset($_GET["MCN"]) && isset($_GET["startDate"]) && isset($_GET["endDate"])) {
    $statement = $conn->prepare("SELECT name, date, start_Time, end_Time 
        FROM schedule 
        WHERE MCN = :MCN AND date BETWEEN :startDate AND :endDate 
        ORDER BY name ASC, date ASC, start_Time ASC");
...
if (isset($_GET["facility"]) &&isset($_GET["action"])&& $_GET['action'] == 'totalHours') {
    $facility = $_GET['facility'];
    $statement = $conn->prepare("SELECT workat.role,SUM(TIME_TO_SEC(TIMEDIFF(schedule.end_Time, schedule.start_Time))) / 3600 AS total_hours
        FROM schedule
...
//Q15
$Facility15=$conn->prepare("SELECT e.first_name, e.last_name, MIN(w.start_time) AS first_day_work, e.date_of_birth, e.email_address, SUM(TIMESTAMPDIFF(HOUR, s.start_Time, s.end_Time)) AS total_hours_scheduled
FROM employees e
